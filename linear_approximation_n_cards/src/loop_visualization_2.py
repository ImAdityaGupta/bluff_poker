...
# Custom colormap for the right heatmap: black → blue → white
...
class StrategyViewer(QtWidgets.QMainWindow):
    def __init__(self, left_data_list, right_shape, strategies, parent=None):
        super().__init__(parent)
        self.setWindowTitle('RL Strategy Visualizer')
        self.strategies = strategies
        self.n = strategies[0].n
        ...
        stacked = np.stack(self.left_data_list, axis=0)
        ...
        self.slider.valueChanged.connect(lambda _: self.slider_timer.start(10))
        ...
        self.on_update_right()  # also populate right heatmap

    def on_slider_change(self, value):
        self.left_index = value
        self.update_left_heatmap()
        self.on_update_right()  # update right side whenever slider moves
    ...
    def on_update_right(self):
        ...
        # build hands (reuse precomputed)
        hands = self.hands

        # select current strategy and weight matrix
        strat = self.strategies[self.left_index]
        pv = strat.prob_vector
        current_W = sum(p * W for p, W in zip(pv, strat.weight_matrices))

        # sample action probabilities for each hand
        probs_list = []
        for hand in hands:
            _, probs, _, _ = strat.sample_action(
                history=history,
        ...
        row_labels = [f"{i}{j}" for (i, j) in hands]
...
def main(strategies):
    ...
    # prepare left data slices from last weight matrix of each strategy
    left_list = [strat.weight_matrices[-1] for strat in strategies]

    # right heatmap has one row per unordered pair with replacement
    right_shape = ((n * (n + 1)) // 2, strategies[0].M)

    app = QtWidgets.QApplication(sys.argv)
    viewer = StrategyViewer(left_list, right_shape, strategies)
    viewer.show()
    sys.exit(app.exec_())


if __name__ == '__main__':
    strategies = [strat_from_weights(1, i, -1) for i in range(100)]
    main(strategies)
